...
# 函数：专为 xtunnel 隧道设计的 BBR v3 + UDP/QUIC 鲁棒性调优
apply_xtunnel_tuning() {
    echo -e "\033[36m正在应用 xtunnel 专属隧道增强优化...\033[0m"
    
    # 1. 强制使用 fq 队列，这是 BBRv3 的最佳拍档，并立即切换网卡队列
    ...
    apply_qdisc_to_active_interfaces "fq" || true

    #  core rmem/wmem max 应放开至 32MB 规格，使 Go 客户端 setsockopt 时不受核心配额打折阻碍
    local buf_max="33554432" 
    # 限制发送队列积压水位，防止 smux 在极端多路复用吞吐下引发严重的 Bufferbloat 延迟抖动
    local lowat="16384" 

    # 2. 核心网络栈级参数调优 (同时照顾 TCP WebSocket 与 UDP WebTransport/QUIC)
    if sudo sysctl -w net.core.rmem_max="$buf_max" > /dev/null \
        && sudo sysctl -w net.core.wmem_max="$buf_max" > /dev/null \
        && sudo sysctl -w net.core.rmem_default="$buf_max" > /dev/null \
        && sudo sysctl -w net.core.wmem_default="$buf_max" > /dev/null \
        && sudo sysctl -w net.ipv4.tcp_rmem="4096 87380 $buf_max" > /dev/null \
        && sudo sysctl -w net.ipv4.tcp_wmem="4096 65536 $buf_max" > /dev/null \
        && sudo sysctl -w net.ipv4.tcp_slow_start_after_idle="0" > /dev/null \
        && sudo sysctl -w net.ipv4.tcp_notsent_lowat="$lowat" > /dev/null \
        && sudo sysctl -w net.ipv4.tcp_limit_output_bytes="4194304" > /dev/null \
        && sudo sysctl -w net.ipv4.udp_mem="65536 131072 262144" > /dev/null \
        && sudo sysctl -w net.ipv4.udp_rmem_min="16384" > /dev/null \
        && sudo sysctl -w net.ipv4.udp_wmem_min="16384" > /dev/null \
        && sudo sysctl -w net.ipv4.tcp_fastopen="3" > /dev/null; then
        echo -e "\033[1;32m✔ xtunnel 增强参数已立即生效\033[0m"
    else
        echo -e "\033[31m✘ 部分参数应用失败，请检查当前系统内核环境是否支持这些 sysctl 项。\033[0m"
        return 1
    fi
    ...
    clean_smart_tuning_conf
    {
        ...
        echo "net.core.rmem_default = $buf_max"
        echo "net.core.wmem_default = $buf_max"
        echo "net.ipv4.tcp_rmem = 4096 87380 $buf_max"
        echo "net.ipv4.tcp_wmem = 4096 65536 $buf_max"
        echo "net.ipv4.tcp_slow_start_after_idle = 0"
        echo "net.ipv4.tcp_notsent_lowat = $lowat"
    ...
    persist_qdisc_module "fq"

    echo -e "\033[1;32m✔ 隧道增强配置已永久写入：$SYSCTL_CONF\033[0m"
    echo -e "\033[36m  重点优化项：\033[0m"
    echo -e "\033[33m  - 禁用慢启动重启 -> 消除长时间空闲后多路复用信道首包传输的阶段性卡顿\033[0m"
    echo -e "\033[33m  - 限制发送队列(lowat) -> 极力防止 Bufferbloat 发生，保障 xtunnel 极速探测活性\033[0m"
    echo -e "\033[33m  - 优化 BDP 与 UDP 缓冲区 -> 彻底解决 WebTransport (QUIC-go) 接收缓冲受限引发的悄然丢包\033[0m"
    echo -e "\033[33m  - 启动 TCP Fast Open (3级) -> 允许物理连接重建握手时直接承载数据帧\033[0m"
}
...
echo -e "\033[33m13. 🛡️  xtunnel 专属增强优化（抗断连/稳4K）\033[0m"
print_separator
echo -n -e "\033[36m请选择一个操作 (1-13) (｡･ω･｡): \033[0m"
read -r ACTION
...
case "$ACTION" in
    ...
    12)
        ...
        ;;
    13)
        echo -e "\033[1;32m(🛡️) 您选择了 xtunnel 专属增强优化！\033[0m"
        apply_xtunnel_tuning
        ;;
    *)
        echo -e "\033[31m(￣▽￣)ゞ 无效的选项，请输入 1-13 之间的数字哦~\033[0m"
        ;;
esac
